...
void Rover::compass_cal_update() {
    ...
    /*TODO: Stick Calibration after arming.is_armed() is implemented
    if (compass.is_calibrating()) {
        if(!arming.is_armed() && g.rc_4.control_in < -4000 && g.rc_3.control_in > 900) {
            compass.cancel_calibration_all();
        }
        return;
    }

    bool stick_gesture_detected = !arming.is_armed() && g.rc_4.control_in > 4000 && g.rc_3.control_in > 900;

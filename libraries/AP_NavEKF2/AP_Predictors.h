...
#define BUFFER_SIZE  15   //  buffer size for sensors, this allows buffering of at least BUFFER_SIZE*20 ms=MAX_MSDELAY of data
...
public:
    ...
    void BestIndex2(uint32_t &closestTime, uint16_t &closestStoreIndex, uint32_t (&timeStamp)[BUFFER_SIZE], uint32_t &currrentTimeStamp_ms, uint16_t &_msecTauDelay);

...
#define Pi 3.1415927f // math Pi constant
...
void NavEKF::SetFlightAndFusionModes()
{
    ...
    if (assume_zero_sideslip()) {
        ...
        uint8_t heightVarying = (fabsf(_baro.get_climb_rate()) > 0.5f); // this will trigger during change in baro height
        uint8_t highGndSpdStage1 = (uint8_t)(gndSpdSq > 9.0f); // trigger at 3 m/s GPS velocity
        uint8_t highGndSpdStage2 = (uint8_t)(gndSpdSq > 36.0f); // trigger at 6 m/s GPS velocity
        uint8_t highGndSpdStage3 = (uint8_t)(gndSpdSq > 81.0f); // trigger at 9 m/s GPS velocity
        uint8_t largeHgt = (uint8_t)(fabsf(hgtMea) > 15.0f); // trigger if more than 15m away from initial height
        uint8_t accelerating = (accNavMag > 0.5f); // this will trigger due to air turbulence during flight
...
void NavEKF::alignYawGPS()
{
    if ((sq(velNED[0]) + sq(velNED[1])) > 25.0f) {
        ...
        badMag = (( fabsf(yawErr) > 0.7854f) && ( fabsf(yawErr) < 5.4978f));
